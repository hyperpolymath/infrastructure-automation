...
firewall, monitoring, networking — everything that runs _on_ your host.
...
Interoperable +
Compatible with link:../ambientops/hybrid-automation-router/[HAR] for
IaC translation.
...
|`+monitoring+` |Monitoring tools and self-monitoring
|`+monitoring/init.sls+`

|`+networking+` |/etc/hosts, hostname, DNS |`+networking/init.sls+`

|`+podman_containers+` |Podman container management
|`+containers/init.sls+`
